...
from core.views import SaleViewSet


class SaleTest(BaseTestCase):
    def test_list_sales_by_user(self):
        self.make_sale(user=self.user)
        for _ in range(5):
            self.make_sale(user=self.not_staff_user)
    ...
    def test_retrieve_sale(self):
        sale = self.make_sale(user=self.not_staff_user)
        req = self.factory.get("/api/sales/")
        force_authenticate(req, user=self.not_staff_user)
    ...
    def test_list_sales_by_user_username(self):
        for i in range(10):
            if i == 0:
                ...
                self.make_sale(user=self.not_staff_user)
        req = self.factory.get(f"/api/sales?search={self.not_staff_user.username}")
        force_authenticate(req, user=self.admin_user)
        response = SaleViewSet.as_view({"get": "list"})(req)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
